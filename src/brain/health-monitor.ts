...
export interface HealthMonitorCallbacks {
	...
	/** Get current harness/runtime state for an arm */
	getArmRuntimeState?: (
		armId: string,
...
export class ArmHealthMonitor {
	...
	private async executeIntervention(
		...
		if (action === "prompt") {
			const suppressionReason = await this.getPromptSuppressionReason(
				armId,
				analysis,
			);
			if (suppressionReason) {
				this.logFn(
					`[HealthMonitor] Skipping prompt for ${armId}: ${suppressionReason}`,
				);
				return null;
			}
		}
	...
	/**
	 * Determine whether a prompt intervention should be suppressed
	 */
	private async getPromptSuppressionReason(
		armId: string,
		analysis: ArmAnalysis,
	): Promise<string | null> {
		...
		// If there is observed recent activity, don't send idle prompts.
		if (
			metrics.recentMessageCount > 0 ||
			metrics.recentToolCount > 0 ||
			metrics.recentFileEditCount > 0
		) {
			return "recent activity detected in event window";
		}

		// Cross-check with runtime state from harness/API.
		if (this.callbacks.getArmRuntimeState) {
			try {
				...
				this.logFn(
					`[HealthMonitor] Failed to fetch runtime state for ${armId}: ${err}`,
				);
			}
		}
	...
	private generatePromptMessage(
		...
		switch (analysis.state) {
			...
			case "looping":
				return (
					`It looks like you might be stuck in a loop (${analysis.loopPattern?.repetitions} repetitions detected). ` +
	...
	private async publishHealthCheckEvent(
		...
		try {
			...
			this.logFn(
				`[HealthMonitor] Failed to publish health check event: ${err}`,
			);
		}
	}
	...
	private async publishInterventionEvent(
		...
		try {
			...
			this.logFn(
				`[HealthMonitor] Failed to publish intervention event: ${err}`,
			);
		}
	}
	...
	updateConfig(config: Partial<HealthMonitorConfig>): void {
		...
		if (
			...
			config.checkIntervalMs !== this.config.checkIntervalMs
		) {
			this.stop();
		}
