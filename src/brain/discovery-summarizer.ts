...
export class DiscoverySummarizer {
   ...
   */
  async summarize(options: SummarizeDiscoveriesOptions): Promise<DiscoverySummary> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        this.logger(`[discovery-summarizer] API error: ${err.substring(0, 200)}`);
        return this.fallbackSummarize(options);
      }
...
`;
   ...
   */
  private fallbackSummarize(options: SummarizeDiscoveriesOptions): DiscoverySummary {
    const { task, globalDiscoveries, taskDiscoveries } = options;
    const allDiscoveries = [...taskDiscoveries, ...globalDiscoveries];

    // Extract exploration discoveries
    const explorationDiscoveries = allDiscoveries.filter((d) => d.phase === "exploration");
    ...
    const relevantFiles = allDiscoveries.filter((d) => d.file).map((d) => d.file!);
    ...
    const missingContext = allDiscoveries.filter((d) => d.kind === "missing_context");
    ...
    const ambiguous = allDiscoveries.filter((d) => d.kind === "ambiguous_requirement");
