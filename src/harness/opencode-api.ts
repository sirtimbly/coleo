...
export class OpenCodeApiHarness implements AgentHarness {
	...
	async spawn(config: SpawnConfig): Promise<HarnessSession> {
		...
		// Read OpenCode auth.json and set API key for the provider
		if (config.provider) {
			try {
				...
				const authKey = config.provider;
				if (auth[authKey]) {
					const envVar = `${config.provider.toUpperCase().replace(/-/g, "_")}_API_KEY`;
					env[envVar] = auth[authKey].key || auth[authKey];
				}
			} catch {
				// Ignore if auth.json doesn't exist or can't be read
			}
		}
		...
		const octopaiDir =
			config.env.OCTOPAI_DIR || process.env.OCTOPAI_DIR || getOctopaiDir();
		...
		console.log(
			`[harness-api] Configuring MCP command: ${bunBinary} run ${cliEntrypoint} mcp serve`,
		);
			...
			mcp: {
				octopai: {
					...
					environment: {
						...
						OPENCOD_API_URL: `http://127.0.0.1:${port}`,
		...
		console.log(
			`[harness-api] Created OpenCode config at ${opencodeConfigPath}${opencodeConfig.model ? ` (model: ${opencodeConfig.model})` : ""}`,
		);

		console.log(
			`[harness-api] Starting OpenCode server on port ${port} for arm ${armId}...`,
		);
		...
		if (this.eventCallbacks.size > 0) {
			const eventStream = new OpenCodeEventStream({
				...
				onError: (error) => {
					console.error(
						`[harness-api] ${armId} event stream error:`,
						error.message,
					);
				},
			});
		...
		console.log(
			`[harness-api] OpenCode API session ${sessionId} started (server session: ${session.id})`,
		);
	...
	private async waitForServer(
		...
		while (Date.now() - startTime < timeoutMs) {
			try {
				...
				if (response.ok) {
					console.log(`[harness-api] Server ready`);
					return;
				}
			} catch {
	...
	async recover(
		...
		try {
			...
			console.log(
				`[harness-api] Cannot connect to server on port ${port}: ${err}`,
			);
			return null;
		}
		...
		try {
			...
			if (!response.ok) {
				console.log(
					`[harness-api] Failed to list sessions: ${response.statusText}`,
				);
				return null;
			}
			...
			if (this.eventCallbacks.size > 0) {
				...
				apiSession.eventStream = eventStream;
				console.log(
					`[harness-api] Started event stream for recovered ${armId}`,
				);
			}
			...
			console.log(
				`[harness-api] Recovered session for ${armId} on port ${port} (session: ${existingSession.id})`,
			);
			return apiSession;
		} catch (err) {
	...
	async kill(session: HarnessSession): Promise<void> {
		...
		try {
			...
			console.log(
				`[harness-api] Could not dispose via API, killing process directly`,
			);
		}
	...
	async sendPrompt(
		...
		if (options?.interrupt) {
			console.log(
				`[harness-api] Aborting current operation before sending prompt`,
			);
		...
		console.log(
			`[harness-api] Sending prompt to ${session.id}: "${prompt.slice(0, 50)}..."`,
		);

		// Use message endpoint with noReply to send asynchronously
		const response = await fetch(
			`${apiSession.serverUrl}/session/${apiSession.sessionId}/prompt_async`,
			{
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({
	...
	async sendPromptSync(
		...
		console.log(
			`[harness-api] Sending sync prompt to ${session.id}: "${prompt.slice(0, 50)}..."`,
		);

		const response = await fetch(
			`${apiSession.serverUrl}/session/${apiSession.sessionId}/message`,
			{
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({
	...
	async interrupt(session: HarnessSession): Promise<void> {
		...
		try {
			const response = await fetch(
				`${apiSession.serverUrl}/session/${apiSession.sessionId}/abort`,
				{
					method: "POST",
				},
			);
	...
	async compact(session: HarnessSession): Promise<void> {
		...
		try {
			const response = await fetch(
				`${apiSession.serverUrl}/session/${apiSession.sessionId}/command`,
				{
					method: "POST",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({
			...
			if (response.ok) {
				console.log(
					`[harness-api] Compacted context for session ${session.id}`,
				);
			}
		} catch (err) {
