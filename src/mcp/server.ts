...
export function createMcpServer(): McpServer {
	...
	server.registerTool(
		...
		async ({
			...
			if (status === "blocked" || status === "issues_found") {
				responseText +=
					" and may adjust the plan or create verification tasks.";
	...
	server.registerTool(
		...
		{
			description:
				"Assign a bug to a specific arm for investigation or fixing. Use this when you need to delegate bug handling to another arm.",
			inputSchema: {
				bug_id: z.string().describe("ID of the bug to assign"),
				assignee_arm_id: z
					.string()
					.describe("ID of the arm to assign this bug to"),
		...
		async ({ bug_id, assignee_arm_id, reason }) => {
			...
			try {
				// Verify bug exists
				const bug = database
					.query("SELECT title FROM bugs WHERE id = ?")
					.get(bug_id) as any;
				if (!bug) {
					return {
						content: [
				...
				if (result.changes > 0) {
					...
					broadcast("bugs", "bug.updated", {
						bugId: bug_id,
						updates: { assigneeArmId: assignee_arm_id },
					});
	...
	server.registerTool(
		...
		{
			description:
				"Acknowledge that you've received and started working on a task",
			inputSchema: {
				task_id: z.string().describe("The ID of the task"),
				screenshot_path: z
					.string()
	...
	server.registerTool(
		...
		{
			description:
				"Report that this arm is still alive and working. Call this periodically (every 30-60 seconds) to let the brain know you're active. The brain will mark arms as stopped if they don't heartbeat for too long.",
			inputSchema: {
				status: z
		...
		async ({ status, current_task }) => {
			...
			logActivity(ARM_ID, "heartbeat", undefined, {
				messageId,
				status: status || "idle",
				current_task,
			});
		...
		"validate_task",
		{
			...
			inputSchema: {
				...
				approved: z
					.boolean()
					.describe(
						"Whether the task completion is approved (true) or rejected (false)",
					),
