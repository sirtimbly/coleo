...
program
  ...
  .action(async (options) => {
    ...
    // Try to get enhanced status from API if available
    type EnhancedStatus = {
      status: string;
      version: string;
      uptime: number;
    ...
    if (enhancedStatus) {
      ...
      console.log(`  Database: ${infra.database.healthy ? "✓ Healthy" : "✗ " + (infra.database.error || "Unhealthy")}`);
      console.log(`  NATS: ${infra.nats.healthy ? "✓ Healthy" : "✗ " + (infra.nats.error || "Unhealthy")}${infra.nats.optional ? " (optional)" : ""}`);
      console.log(`  Maildir: ${infra.maildir.healthy ? "✓ Healthy" : "✗ " + (infra.maildir.error || "Unhealthy")}`);
      console.log(``);

      // Arms with health details
      ...
      if (arms.total > 0) {
        ...
        if (arms.details && arms.details.length > 0) {
          ...
          for (const arm of arms.details) {
            const healthIcon = arm.health === "healthy" ? "✓" : arm.health === "idle" ? "○" : arm.health === "stuck" ? "✗" : "△";
