...
let prevFrameNow = 0
let lastMoving: { x:number, y:number, angle:number, speed:number, t:number } | null = null
...
function getMouseAngleOver(windowMs: number, nowTs = performance.now()) {
  ...
  let dx = 0, dy = 0
  for (let i = 1; i < pts.length; i++) { dx += pts[i].x - pts[i-1].x; dy += pts[i].y - pts[i-1].y }
...
function spawnSwirlsAt(centerX:number, centerY:number, dirAngle:number, initSpeed:number, side:number, countOverride?:number, opts: any = {}){
  ...
  for (let i=0;i<count;i++){
    ...
    swirls.push({
      ...
      straightX:centerX, straightY:centerY,
      turned:false, turnedAt:0, baseOmega:0, swirlBurstMs:250, dir:curlSide,
      trail:[{x:ox,y:oy,t:0}], prevTime:created
    })
  }
}
...
function maybeSpawnEddyOnTurn(nowTs=performance.now()){
  ...
  if (!prevAngle){ (maybeSpawnEddyOnTurn as any).prevAngle=newAngle; (maybeSpawnEddyOnTurn as any).prevSpeed=getMouseSpeedOver(200,nowTs); return }
  ...
  if (ENABLE_BURST && lastMoving){
    ...
    spawnSwirlsAt(lastMoving.x, lastMoving.y, lastMoving.angle, lastMoving.speed, side, 12, {straightDist})
  }
...
function updateDepth(){
  ...
  ;(waterLayer as HTMLElement).style.filter = `brightness(${bgBrightness}) saturate(${bgSaturation})`
}

function initAnimation(){
  raysCanvas = document.getElementById('raysCanvas') as HTMLCanvasElement
  sparklesCanvas = document.getElementById('sparklesCanvas') as HTMLCanvasElement
  ...
  if(raysCanvas){ raysCanvas.width=viewportWidth.value; raysCanvas.height=viewportHeight.value }
  if(sparklesCanvas){ sparklesCanvas.width=viewportWidth.value; sparklesCanvas.height=viewportHeight.value }

  rays=[]
  for(let i=0;i<8;i++){
    rays.push({ x:(viewportWidth.value/8)*i+(Math.random()-0.5)*40, y:-200-Math.random()*200, width:120+Math.random()*80, length:viewportHeight.value*1.5, speed:0.1+Math.random()*0.2, phase:Math.random()*Math.PI*2, opacity:0.08+Math.random()*0.1 })
  }
  sparkles=[]
  for(let i=0;i<SPARKLE_COUNT;i++){
    sparkles.push({ x:Math.random()*viewportWidth.value*2, y:Math.random()*docHeight.value, radius:2+Math.random()*3, speed:0.5+Math.random()*1.0, wobble:Math.random()*Math.PI*2, wobbleSpeed:0.01+Math.random()*0.02, phase:Math.random()*Math.PI*2, maxOpacity:0.4+Math.random()*0.4 })
  }
  ...
  window.addEventListener('mousemove',(e)=>{
    mouse.value.x=e.clientX; mouse.value.y=e.clientY; mouse.value.active=true
    const now=performance.now()
    mouseHistory.push({x:e.clientX,y:e.clientY,t:now})
    while(mouseHistory.length && (now-mouseHistory[0].t)>1000) mouseHistory.shift()
  })
  window.addEventListener('mouseleave',()=>{ mouse.value.active=false })
  window.addEventListener('scroll',()=>{
    scrollY.value=window.scrollY
    const parallax=-scrollY.value*0.2
    if(waterLayer) (waterLayer as HTMLElement).style.transform=`translate3d(0,${parallax}px,0)`
    if(raysCanvas) (raysCanvas as HTMLCanvasElement).style.transform=`translate3d(0,${parallax}px,0)`
  })
...
function animate(){
  ...
  if(sparklesCanvas){
    ...
    if(mouse.value.active){
      const k=8, damp=Math.max(0,1-4*dt)
      const mx=mouse.value.x, my=scrollY.value+mouse.value.y
      const ax=(mx-mass.x)*k, ay=(my-mass.y)*k
      mass.vx=(mass.vx+ax*dt)*damp; mass.vy=(mass.vy+ay*dt)*damp
      mass.x+=mass.vx*dt; mass.y+=mass.vy*dt
    }
    ...
    if(speedNow>120){ lastMoving={ x:mouse.value.x, y:scrollY.value+mouse.value.y, angle:angNow!, speed:speedNow, t:nowTs } }
    // Turn/stop bursts
    maybeSpawnEddyOnTurn(nowTs)

    // Continuous streaks
    const SPEED_THRESHOLD=90
    if(speedNow>SPEED_THRESHOLD && nowTs>=nextContinuousSpawnAt && nowTs>=newDirectionCooldownUntil){
      const mx=mouse.value.x, my=scrollY.value+mouse.value.y
      const streakCount=Math.random()<0.3?2:1
      const clampedSpeed=Math.min(MAX_STREAK_SPEED,speedNow)
      for(let i=0;i<streakCount;i++) spawnStreak(mx,my,angNow!,clampedSpeed)
    ...
    let reefRGB=waterStyle.getPropertyValue('--reef-rgb').trim() || '73, 215, 175'
    swirls = swirls.filter((s:any)=>{
      const lifeT=(nowTs-s.created)/(s.lifetime)
      if(lifeT>=1) return false
      const dtLocal=Math.max(0.001,(nowTs - (s.prevTime||nowTs))/1000); s.prevTime=nowTs
      ...
      if(!inStraight && !s.turned){
        ...
        s.radVel *= 0.6
        s.turnedAt=nowTs; s.baseOmega=(2.2+Math.random()*1.2)*curlSign; s.cx=s.straightX; s.cy=s.straightY; s.turned=true
      }
      ...
      if(!inStraight && s.turned){
        ...
        if(swirlAge < (s.swirlBurstMs||250)){
          ...
          effAng=sign*mag
          const cap=s.maxRad||38, target=s.targetRad||30
      ...
      const vx0=s.cVx||0, vy0=s.cVy||0
      const v0=Math.hypot(vx0,vy0)
      if(v0>0){ const v1=Math.max(0, v0-DRIFT_DRAG_COEFF*v0*v0*dtLocal); const scale=v0>0? v1/v0:0; s.cVx=vx0*scale; s.cVy=vy0*scale }
      const prevCx=s.cx||0, prevCy=s.cy||0
      if(inStraight){ const step=(s.forwardSpeed||200)*dtLocal; s.forwardDist=(s.forwardDist||0)+step; s.straightX+=Math.cos(s.baseAngle||0)*step; s.straightY+=Math.sin(s.baseAngle||0)*step }
      else { s.cx+=(s.cVx||0)*dtLocal; s.cy+=(s.cVy||0)*dtLocal; const dtx=(s.cx||0)-prevCx, dty=(s.cy||0)-prevCy; if(Math.abs(dtx)+Math.abs(dty)>0){ for(let k=0;k<s.trail.length;k++){ s.trail[k].x+=dtx; s.trail[k].y+=dty } } const curlSide=s.dir||1; const perpA=(s.baseAngle||0) - curlSide*Math.PI/2; const ux=Math.cos(perpA), uy=Math.sin(perpA); const segDrift=PERP_SEG_DRIFT*dtLocal; for(let k=0;k<s.trail.length;k++){ s.trail[k].x+=ux*segDrift; s.trail[k].y+=uy*segDrift } }
      s.rad+=s.radVel*dtLocal*ramp*radialFactor; if(s.rad>cap) s.rad=cap; s.angle+=effAng*dtLocal*ramp
      ...
      let startIdx=0, acc=0; const hideLen=totalLen*0.2; for(let i=1;i<s.trail.length;i++){ const a=s.trail[i-1],b=s.trail[i]; const seg=Math.hypot(b.x-a.x,b.y-a.y); if(acc+seg>=hideLen){ startIdx=i; break } acc+=seg }
      ...
      if(s.trail.length-startIdx>=2 && visibleLen>0){
        ...
        for(let i=startIdx+1;i<s.trail.length;i++){
          ...
          const segLen=Math.hypot(b.x-a.x,b.y-a.y); accVis+=segLen
          ...
          let nx=b.y-a.y, ny=-(b.x-a.x); const nlen=Math.hypot(nx,ny)||1; nx/=nlen; ny/=nlen
          const seed=Math.sin(i*12.9898 + s.created*0.001)*43758.5453
          const j=((seed-Math.floor(seed))*2-1)*1.5
          const ax=a.x+nx*j, ay=a.y+ny*j; const bx=b.x+nx*j, by=b.y+ny*j
          const gSeg=ctx.createLinearGradient(ax, ay-scrollY.value, bx, by-scrollY.value)
          gSeg.addColorStop(0,`rgba(${reefRGB}, 0)`); gSeg.addColorStop(0.5,`rgba(${reefRGB}, ${alpha})`); gSeg.addColorStop(1,`rgba(${reefRGB}, 0)`)
          ctx.beginPath(); ctx.moveTo(ax, ay-scrollY.value); ctx.lineTo(bx, by-scrollY.value)
          ctx.strokeStyle=gSeg; ctx.lineWidth=1.2; ctx.lineCap='round'; ctx.lineJoin='round'; ctx.shadowColor=`rgba(${reefRGB}, ${alpha*0.5})`; ctx.shadowBlur=1.5
          ctx.stroke()
        }
      }
      return true
    ...
    // Streaks
    const streakAlphaBase=0.40, streakWidth=1.25
    streaks = streaks.filter((s:any)=>{
      const life=nowTs-s.created; const t=life/s.lifetime; if(t>=1) return false
      s.px=s.x; s.py=s.y; s.speed=Math.max(0, s.speed - STREAK_DRAG_COEFF*s.speed*s.speed*dt)
      const step=s.speed*dt; s.x+=Math.cos(s.angle)*step; s.y+=Math.sin(s.angle)*step
      const dy1=s.py - scrollY.value, dy2=s.y - scrollY.value
      ctx.beginPath(); ctx.moveTo(s.px, dy1); ctx.lineTo(s.x, dy2)
      ctx.strokeStyle=`rgba(255,255,255,${streakAlphaBase*(1-t)})`; ctx.lineWidth=streakWidth; ctx.lineCap='round'
...
onMounted(()=>{ initAnimation(); animate() })
