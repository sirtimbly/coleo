...
Coleo 0.1 marks the first release we'd confidently recommend for real use.
...
We've invested heavily in the parts that typically break first in multi-agent tooling:
...
That's exactly why we called it 0.1. The core is strong enough to use. The edges are still being shaped.
