...
- `Dockerfile.cloudflare-agent` runs NATS/JetStream and `coleo agent start`.

This is a real remote-agent topology. The control image must set:
...
The agent image uses its local TCP NATS listener:
...
is passed to every NATS connection, including the API server, arm agent, MCP
tools, and transcript indexer.

When `COLEO_REMOTE_ARMS_ONLY=1`, all harnesses are forced through the registered
arm agent and local fallback is disabled. The control container therefore never
...
The agent exposes TCP `4222`, monitoring `8222`, and WebSocket `9222`. Only the
monitor and WebSocket ports need to be made reachable through the Cloudflare
Container binding; TCP remains local to the agent image. TLS is terminated by
the edge Worker before port `9222`.

Both images synchronize state to separate R2 prefixes. The control image uses
`control/`; the agent uses `agent/` and uploads its JetStream store only during
a graceful shutdown.
