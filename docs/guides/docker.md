...
The repository is cloned into `/home/coleo/projects/app`. The `arm-agent` service starts from that directory and advertises OpenCode capabilities over NATS, so arms created from the web UI can run headlessly through the `opencode-api` harness.
